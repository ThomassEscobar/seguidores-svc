# Seguidores — Arquitectura del microservicio

## Componentes

| Componente | Rol |
|---|---|
| `SeguimientoApplication` | Punto de entrada Spring Boot |
| `controller/SeguimientoController.java` | API REST expuesta en `/api/seguimientos` |
| `service/SeguimientoService.java` | Lógica de negocio y transacciones |
| `repository/SeguimientoRepository.java` | Acceso JPA a H2 |
| `model/Seguimiento.java` | Entidad persistida en tablas H2 |
...
`Cliente/Sistema → API Gateway → SeguimientoController → SeguimientoService → SeguimientoRepository → H2`
